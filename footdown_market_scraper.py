class FwMspider():
    ...
    def _getMain(self):
        ...
        for url in data:
            ...
            if res.status_code == 200:
                print('Success...')
                temp = httpx.get('https://api.freshop.com/1/products?app_key=foodtown&department_id_cascade=true&include_departments=true&limit=0&render_id=1665284471218&store_id=2601&token=125457c17162ab595f65d101acd593f4',headers=headers)
    ...
    def parseData(self):
        ...
        if self.productIds:
            for id_ in self.productIds:
                res = httpx.get(url='https://api.freshop.com/1/products/%s?app_key=foodtown&render_id=1665289910016&store_id=2601&token=125457c17162ab595f65d101acd593f4'%id_[0],headers=headers)
                if res.status_code == 200:
                    result = res.json()
                    # ******************************************************************************************* #
    ...
    def Save(self):
        pd.DataFrame(self.Data).to_csv('%d '%len(self.productIds)+self.Path,header=self.header,index=False,encoding='utf-8')
